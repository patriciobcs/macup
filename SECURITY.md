...
- **Talks to a fixed set of hosts over HTTPS.** Package registries (npm, PyPI, crates.io, RubyGems, Packagist, the Go module proxy), GitHub (release lookups and Homebrew commit dates), OSV.dev (advisories) and, for downloaded copies, the Sparkle appcast on this repository's releases. The package managers themselves contact their own registries.
