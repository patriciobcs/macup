...
const bar =
  "flex h-7 items-center text-[14px] leading-7 text-white antialiased drop-shadow-[0_1px_1px_rgba(0,0,0,0.35)]";
...
export function StatusCluster({ count }: { count: number }) {
  return (
    <div className={`${bar} justify-end gap-[16px]`}>
      <span
        className="flex h-[24px] items-center gap-1.5 rounded-[6px] bg-white/25 px-1.5"
        aria-label={`MacUp: ${count} updates ready`}
      >
