...
enum Screenshots {
    static func runIfRequested() -> Bool {
        ...
        for dark in [false, true] {
            ...
            capture(
                SettingsView().environment(store).environment(settings),
                size: CGSize(width: 520, height: 700), dark: dark,
    ...
    private static func capture<V: View>(_ view: V, size: CGSize?, dark: Bool, to url: URL) {
        ...
        let hosting = NSHostingView(
            rootView: view.background(Color(nsColor: size == nil ? .underPageBackgroundColor : .windowBackgroundColor)))
        let window = NSWindow(
            contentRect: NSRect(origin: .zero, size: size ?? CGSize(width: 320, height: 600)),
        ...
        guard let content = window.contentView, let rep = content.bitmapImageRepForCachingDisplay(in: content.bounds)
        else {
            window.close()
            return
        }
...
extension UpdateStore {
    ...
    static func fixture() -> UpdateStore {
        ...
        let now = Date()
        func pkg(
            _ m: Manager, _ name: String, _ from: String, _ to: String, kind: String = "formula",
            ago: (released: Double, updated: Double), advisories: [String] = []
        ) -> OutdatedPackage {
            var p = OutdatedPackage(manager: m, name: name, installed: from, latest: to, kind: kind, extra: "")
            p.releaseDate = now.addingTimeInterval(-ago.released * 86_400)
            p.dateSource = .registry
            p.updatedAt = now.addingTimeInterval(-ago.updated * 86_400)
            p.advisories = advisories
            return p
        }
        store.loadFixture(
            reports: Manager.allCases.map {
                ManagerReport(
                    manager: $0,
                    status: [.macos, .brew, .npm, .bun, .pip, .uv, .cargo, .rustup, .gem, .tools].contains($0)
                        ? .ok : .missing, message: "")
            },
            packages: [
                pkg(.brew, "ghostty", "1.2.3", "1.3.1", kind: "cask", ago: (6, 40)),
                pkg(.brew, "jq", "1.7.1", "1.8.2", ago: (12, 120)),
                pkg(.npm, "npm", "10.9.7", "12.0.2", kind: "global", ago: (30, 200)),
                pkg(.npm, "corepack", "0.34.6", "0.36.0", kind: "global", ago: (9, 200)),
                pkg(.bun, "wrangler", "4.91.0", "4.129.0", kind: "global", ago: (2, 65)),
                pkg(.uv, "modal", "1.5.4", "1.5.5", kind: "tool", ago: (8, 18)),
                pkg(
                    .cargo, "cargo-nextest", "0.9.67", "0.9.143", kind: "crate", ago: (35, 400),
                    advisories: ["RUSTSEC-2026-0012"]),
                pkg(
                    .rustup, "stable-aarch64-apple-darwin", "1.93.0", "1.98.1", kind: "toolchain", ago: (5, 230)),
                pkg(.tools, "uv", "0.7.8", "0.12.10", kind: "tool", ago: (4, 470)),
                pkg(.npm, "happy", "1.1.8", "1.2.3", kind: "global", ago: (0.4, 3)),
            ],
            log:
