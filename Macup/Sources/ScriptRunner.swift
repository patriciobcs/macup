...
enum ScriptRunner {
    ...
    static func scan(managers: [Manager], brewGreedy: Bool) async throws -> ScanResult {
        guard let url = scriptURL("macup-scan") else { throw SubprocessError.launchFailed("macup-scan.sh missing from bundle") }
        let env = await environment(brewGreedy: brewGreedy)
        let r = try await Subprocess.run(executable: "/bin/zsh",
                                         arguments: [url.path] + managers.map(\.rawValue),
                                         environment: env, timeout: 300)
        return ScanParser.parse(r.stdout)
    }

    static func upgrade(manager: Manager, arguments: [String], brewGreedy: Bool,
                        onOutput: @Sendable @escaping (String) -> Void) async throws -> Int32 {
        guard let url = scriptURL("macup-upgrade") else { throw SubprocessError.launchFailed("macup-upgrade.sh missing from bundle") }
        let env = await environment(brewGreedy: brewGreedy)
        let r = try await Subprocess.run(executable: "/bin/zsh",
    ...
    static func remove(pkg: OutdatedPackage, brewGreedy: Bool,
                       onOutput: @Sendable @escaping (String) -> Void) async throws -> Int32 {
        guard let url = scriptURL("macup-remove") else { throw SubprocessError.launchFailed("macup-remove.sh missing from bundle") }
        let env = await environment(brewGreedy: brewGreedy)
        let r = try await Subprocess.run(executable: "/bin/zsh",
    ...
    static func setup(tool: String, onOutput: @Sendable @escaping (String) -> Void) async throws -> Int32 {
        guard let url = scriptURL("macup-setup") else { throw SubprocessError.launchFailed("macup-setup.sh missing from bundle") }
        let env = await environment(brewGreedy: false)
        let r = try await Subprocess.run(executable: "/bin/zsh", arguments: [url.path, tool],
